...
was too high. A 30,000-step fit with 90 windings (track-radius loss 123 vs 182 at 10,000 steps with 130) renders clean continuous sheets, e.g. winding w089 over rows 20-60 (`pherc0125_w089_render.png`, 8 mm x 41 mm); its ink map is empty (`pherc0125_w089_ink.png`). The sweep of every third winding of that mesh (24 windings, 7.5 mm band, full circumference) is in `results/p0125_batch_acw30k`: all negative, max frac>200 0.0006. A second 1.5 mm z-band 3.5 mm higher (slices 12500-14000) was fitted and swept the same way (`results/p0125_b2_batch`, 23 windings): all negative, max frac>200 0.0009; the innermost winding w021 gives a diffuse blotchy map with no letter forms (`w021_ink.png`). The PHerc0826 second band (slices 6500-8000, `results/p0826_b2_batch`, 17 windings) is likewise negative; its highest winding w033 (robust score 7 blobs, frac>200 0.0038, just under the speckle ceiling) is a blotchy map without letter forms (`w033_ink.png`). The PHerc0211 second band (slices 13500-15000, `results/p0211_b2_batch`, 23 windings) is negative as well (max frac>200 0.0019 at w045). Two further scrolls were fitted from estimated umbilici: PHerc0358 (slices 8000-9500, 100 windings, 50.4% track points satisfied) and PHerc0257 (slices 10000-11500, 90 windings, 51.7%); the PHerc0358 sweep (`results/p0358_batch`, 27 windings) is negative: its two highest windings w072 and w075 (8 letter-sized blobs each, frac>200 0.0019-0.0023, robust rescoring up to 11 blobs at 0.0035) sit where the render crosses a crushed, folded multi-layer region (`w075.zarr_mid_ds2.png`), and the maps there are dense speckle without letter forms, the same failure mode as PHerc0826 w090. The PHerc0257 sweep (`results/p0257_batch`, 46 renders, run locally after the fit) is negative as well: max frac>200 0.0023 (w033, reverse direction, 3 robust blobs), best raw blob count 4 at w039, no strokes or rows in any map. The PHerc0813 sweep (`results/p0813_batch`, 46 renders, fitted and swept entirely on free Kaggle T4 GPUs with the recipe in `kaggle/` and `kaggle_sweep/`) is negative: max frac>200 0.0013 (w045), robust blob count at most 4, again without letter forms. The PHerc0191 sweep (`results/p0191_batch`, 54 renders, also fitted and swept on Kaggle) is negative: max frac>200 0.0011 (w093), robust blob count at most 6 in the reverse direction, no letter forms. That completes the eight track-ready scrolls the estimated-umbilicus recipe was written for: all negative at the flat model's calibrated threshold.
...
| PHerc0358, 30k-step spiral mesh (100 windings, estimated umbilicus, 50.4% tracks), windings w021-w099 (every 3rd), rows 20-60 (7.5 mm band) | 54 renders | 8 (w072, w075: crushed multi-layer region, see caveat) | 0.0023 |
...
| PHerc0800, 30k-step spiral mesh (slices 11000-12500, 100 windings, CW, estimated umbilicus, 57.0% track points, local RTX 3090; volume 20250521135224 at 8.640 um), windings w021-w099 (every 3rd), rows 20-60 (7.5 mm band) | 54 renders | 7 (w099; frac 0.0007) | 0.0036 (w030 reverse: diffuse blotches and a render streak, no letter forms) |
