...
n_layers = int(sys.argv[5]) if len(sys.argv) > 5 else 21      # layers sampled along the sheet normal
surf_scale = int(sys.argv[6]) if len(sys.argv) > 6 else 1     # surface prediction is this factor coarser than CT
os.makedirs(out, exist_ok=True)
def load(p):
    g = zarr.open(p, mode='r'); return np.asarray(g['0'] if hasattr(g, 'keys') and '0' in g else g)
ct = load(ct_path); surf = load(surf_path)
if surf_scale > 1:
    surf = ndi.zoom(surf.astype(np.float32), surf_scale, order=1).astype(np.uint8)
    surf = surf[:ct.shape[0], :ct.shape[1], :ct.shape[2]]
...
sm = ndi.gaussian_filter(rsf, (2, 2, max(1, surf_scale // 2)))
...
for x0 in cand_x:
    ...
    while q:
        ...
        for dz, dy in ((1,0),(-1,0),(0,1),(0,-1)):
            ...
            visited[jz, jy] = True
            zz, yy = fz[jz], fy[jy]; lo, hi = int(max(0, xprev-6*max(1,surf_scale//2))), int(min(X, xprev+7*max(1,surf_scale//2)))
...
for i, (x0, full, vmask, cov_) in enumerate(sheets):
    ...
    Image.fromarray(layers[half]).save(os.path.join(out, name + '_mid.png'))
